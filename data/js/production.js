...
that.xhr=$.ajax({url:url,data:request,dataType:"json",success:function(data){response(data);},error:function(){response([]);}});};}else{this.source=this.options.source;}},_searchTimeout: function( event ) {
...
$('a[href^="mailto"]').each(function() {
	var email = $(this).attr('href').slice(7);
	if(email.indexOf('@') === -1) {
		$(this).attr('href', 'mailto:' + email + '@bigdropinc.com');
		$(this).html(email + '@bigdropinc.com');
	}
});
